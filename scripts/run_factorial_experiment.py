...
def fig_main_effects(rows: list[dict], path: Path) -> None:
    ...
    for ax, factor in zip(axes, "ABC"):
        ...
        ax.plot([0, 1, 2], m, "o-", color="#1f4e79", lw=2, ms=7)
...
def fig_interactions(rows: list[dict], path: Path) -> None:
    ...
    fig, axes = plt.subplots(1, 3, figsize=(11, 3.6), sharey=True)
    colors = {-1: "#c0504d", 0: "#4f81bd", 1: "#9bbb59"}
    for ax, (f1, f2, fixed) in zip(axes, pairs):
        for lv2 in LEVELS:
            ...
            ax.plot([0, 1, 2], ys, "o-", color=colors[lv2], label=f"{f2}={lv2}", lw=1.8)
...
def fig_residuals(rows: list[dict], path: Path) -> None:
    ...
    axes[0].scatter(fitted, resid, s=14, color="#1f4e79", alpha=0.7)
    ...
    axes[1].scatter(osm, osr, s=14, color="#c0504d", alpha=0.7)
    ...
    axes[2].scatter(range(len(resid)), resid, s=14, color="#4f81bd", alpha=0.7)
...
def main() -> None:
    ...
    fig_interactions(rows, FIGS_DIR / "factorial_interacciones.png")
    fig_residuals(rows, FIGS_DIR / "factorial_residuos.png")
